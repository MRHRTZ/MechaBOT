...
let blokir = []
// let metaboard = JSON.parse(fs.readFileSync(`./lib/database/minesweep-${from}.json`))
let groupMines = JSON.parse(fs.readFileSync('./lib/database/group-minesweeper.json'))

function INFOLOG(info) {
     return console.log('\x1b[1;35m>>\x1b[1;37m>', '[\x1b[1;33mINF\x1b[1;37m]', time, color(info))
}
function ERRLOG(e) {
     return console.log('\x1b[1;31m>>\x1b[1;37m>', '[\x1b[1;31mERROR\x1b[1;37m]', time, color(e))
}
...
async function mulai() {
     ...
     conn.on('credentials-updated', () => {
          ...
          fs.writeFileSync('./sessions/MRHRTZ.sesi.json', JSON.stringify(authInfo, null, 2))
     })
     fs.existsSync('./sessions/MRHRTZ.sesi.json') && conn.loadAuthInfo('./sessions/MRHRTZ.sesi.json')
     ...
     conn.on('chat-update', async (chat) => {
          ...
          // if (self) return
          // console.log(hurtz)
          if (!isGroup && isCmd) console.log('\x1b[1;31m>>\x1b[1;37m>', '[' + chalk.blueBright('CMD') + ']', time, color(body), 'dari', color(pushname), 'Dengan ID', color(hurtz.key.id), 'Urutan', color(urutan_pesan))
          if (!isGroup && !isCmd) console.log('\x1b[1;31m>>\x1b[1;37m>', '[' + chalk.greenBright('MSG') + ']', time, color(body), 'dari', color(pushname), 'Dengan ID', color(hurtz.key.id), 'Urutan', color(urutan_pesan))
          if (isCmd && isGroup) console.log('\x1b[1;31m>>\x1b[1;37m>', '[' + chalk.blueBright('CMD') + ']', time, color(body), 'dari', color(pushname), 'di', color(groupName), 'Dengan ID', color(hurtz.key.id), 'Urutan', color(urutan_pesan))
          if (!isCmd && isGroup) console.log('\x1b[1;31m>>\x1b[1;37m>', '[' + chalk.greenBright('MSG') + ']', time, color(body), 'dari', color(pushname), 'di', color(groupName), 'Dengan ID', color(hurtz.key.id), 'Urutan', color(urutan_pesan))
          ...
          // console.log(hurtz)
          if (!self && sender != '6285559038021@s.whatsapp.net') return
...
_Mohon tunggu beberapa menit untuk mengirim file tersebut.._`
                    ...
                    conn.sendMessage(from, pesan_bot, TypePsn.text, {
                         quoted: {
                              key: { remoteJid: nomer_asal.replace('@', '') + '@s.whatsapp.net', fromMe: false },
                              message: { conversation: pesan }
                         }
                    }).then(a => console.log(a.message))
...
${hasil.grid}
                    ...
                    if (args.length === 1) {
                         ...
                         createExif('Created By DGC Bots', 'MRHRTZ as Dev')
...
`
                    ...
                    try {
                         getApkReal('https://rexdlfile.com/index.php?id=' + body.slice(16)).then(res => {
                              ...
                              conn.sendMessage(from, data, TypePsn.document, { caption: res.title, mimetype: extmim, quoted: hurtz})
                              })
                         })
                    } catch (error) {
                         console.log(error)
...
_Mohon tunggu beberapa menit untuk mengirim file tersebut.._`
                    ...
                    conn.sendMessage(from, body.slice(9), TypePsn.text, {
                         text: 'ok',
                    ...
                    webp2mp4File(savedFilename).then(async (rest) => {
                         sendDariUrl(from, rest.result, TypePsn.video, `Dahh jadi ni ${pushname}`)
                         fs.unlinkSync(savedFilename)
                    }).catch((e) => {
                         console.log(e)
                         balas(from, `Error gan :(`)
                    ...
                    const packstik = body.slice(8).split('|')[0] || 'DGC ChatBotV3'
                    const authorstik = body.split('|')[1] || 'MRHRTZ as Dev'
                    ...
                    if (savedFilename.slice(-4) === 'jpeg') {
                         exec(`ffmpeg -i ${savedFilename} -vcodec libwebp -filter:v fps=fps=20 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ./media/sticker/${filename}.webp`, (err, stdout, stderr) => {
...
💛 !gtaV <TagGambar>
     ...
     conn.on('message-update', async (hurtz) => {
          try {
               const from = hurtz.key.remoteJid
               const messageStubType = WA_MESSAGE_STUB_TYPES[hurtz.messageStubType] || 'MESSAGE'
               const dataRevoke = JSON.parse(fs.readFileSync('./lib/database/RevokedGroup.json'))
