               ...
               dgc.latest(1)
                    .then((resultA) => {
                         ...
                         if (db_blog.all_post < resultA.all_post) {
                              console.log(chalk.greenBright('[ MECHABOT ]  ') + moment(new Date()).format('HH:mm:ss DD/MM/YYYY') + ' ' + chalk.white('Status Changed', totalUpdate, 'Content'))
                              db_blog.status = true
                         ...
                         if (db_blog.status) {
                              ...
                              console.log(chalk.greenBright('[ MECHABOT ]  ') + moment(new Date()).format('HH:mm:ss DD/MM/YYYY') + ' ' + chalk.bgGrey('Update Content : ' + resultA.result[0].title + ' - ' + resultA.result[0].author_post))
...
*_☠️ Powered By DGC BOT ☠️_*`
                              ...
                              console.log(chalk.greenBright('[ MECHABOT ]  ') + moment(new Date()).format('HH:mm:ss DD/MM/YYYY') + ' ' + chalk.bgGrey('No post update'))
                         }
                    })
          }
          ...
          switch (cmd) {
               ...
               case 'kirim':
                    ...
                    try {
                         ...
                         await conn.sendMessage(args[1].includes('@g.us') ? args[1] : args[1] + '@s.whatsapp.net', args.slice(2).join(' '), 'conversation')
                    ...
                    break
               case 'test':
                    const tst = clients.some(arr => client_log.includes(arr))
                    // ws.send(util.format(client_log))
                    ws.send('test : ' + tst)
                    break
               case 'cekdata':
                    checkAndExecute()
