...
function fbdl(url) {
     return new Promise((resolve, reject) => {
          Axios.get('https://mrhrtz-wabot.000webhostapp.com/FB/apifb.php?url=' + url)
               .then(({ data }) => {
                    if (data.title === '') return reject("Can't access media")
                    resolve({
                         status: true,
                         title: data.title,
                         download: [
                              data.links.Download_Low_Quality,               // LOW RESOLUTION
                              data.links.Download_High_Quality               // HIGH RESOLUTION
                         ]
                    })
               })
               .catch(reject)
