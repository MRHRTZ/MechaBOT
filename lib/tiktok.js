...
function ttdownload(url) {
     return new Promise((resolve, reject) => {
          Axios.get(`https://ttdownloader.com/?url=${url}`)
               .then((data) => {
                    const $ = cheerio.load(data.data)
                    const token = $('input#token').attr('value')
                    // console.log(data.headers['set-cookie'].join(''));
                    Axios({
                         ...
                         const result = {
                              status: true,
                              [$('#results-list > div:nth-child(2) > div.size').text().replace(/ +/g, '')]: $('#results-list > div:nth-child(2) > div.download > a').attr('href'),
                              [$('#results-list > div:nth-child(3) > div.size').text().replace(/ +/g, '')]: $('#results-list > div:nth-child(3) > div.download > a').attr('href'),
                              [$('#results-list > div:nth-child(4) > div.size').text().replace(/ +/g, '')]: $('#results-list > div:nth-child(4) > div.download > a').attr('href')
                         }
                         resolve(result)
                    })
...
// ttdownload('https://www.tiktok.com/@youneszarou/video/6942436555692805381')
...
module.exports.ttdownload = ttdownload
